...
#ifndef CONTIGUOUS_ARRAYS_H				
#define CONTIGUOUS_ARRAYS_H


template <class T> void allocate_array_GPU(T *&pointer, int dim1){
  cudaCheckError(cudaMalloc(&pointer, dim1 * sizeof(T)));
}

template <class T> void allocate_array_GPU(T *&pointer, int dim1, int  dim2){
  cudaCheckError(cudaMalloc(&pointer, dim1 * dim2 *sizeof(T)));

}

template <class T> void allocate_array_GPU(T *&pointer, int dim1, int  dim2,int dim3){
  cudaCheckError(cudaMalloc(&pointer, dim1 * dim2* dim3 *sizeof(T)));

}

template <class T> void allocate_array_GPU(T *&pointer, int dim1, int  dim2,int dim3, int dim4){
  cudaCheckError(cudaMalloc(&pointer, dim1 * dim2 * dim4 * sizeof(T)));
