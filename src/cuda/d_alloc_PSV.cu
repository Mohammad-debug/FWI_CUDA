...
void alloc_varmain_PSV_GPU(
    ...
    // allocate velocities
    allocate_array_GPU(vz, nz, nx); 
    allocate_array_GPU(vx, nz, nx); 

    // allocate displacements
    allocate_array_GPU(uz, nz, nx); 
    allocate_array_GPU(ux, nz, nx); 

    // allocate stress tensors
    allocate_array_GPU(szz, nz, nx); 
    allocate_array_GPU(szx, nz, nx); 
    allocate_array_GPU(sxx, nz, nx); 

    // Allocate Energy for forward modelling
    allocate_array_GPU(We, nz, nx); 

    // spatial derivatives 
    // (same arrays can be used for ...
    // ...velocity and stress computations)
    allocate_array_GPU(dz_z, nz, nx);
    allocate_array_GPU(dx_z, nz, nx);
    allocate_array_GPU(dz_x, nz, nx);
    allocate_array_GPU(dx_x, nz, nx);

    // Allocating PML memory array for spatial derivatives
    if (pml_z){ // PMLs in z-direction
        allocate_array_GPU(mem_vz_z, nz, nx);
        allocate_array_GPU(mem_vx_z, nz, nx);
        allocate_array_GPU(mem_szz_z, nz, nx);
        allocate_array_GPU(mem_szx_z, nz, nx);
    }
    
    if (pml_x){ // PMLs in x-direction
        allocate_array_GPU(mem_vz_x, nz, nx);
        allocate_array_GPU(mem_vx_x, nz, nx);
        allocate_array_GPU(mem_szx_x, nz, nx);
        allocate_array_GPU(mem_sxx_x, nz, nx);
    }

    // Allocates material average arrays
    allocate_array_GPU(mu_zx, nz, nx);
    allocate_array_GPU(rho_zp, nz, nx);
    allocate_array_GPU(rho_xp, nz, nx);


    // rtf (used as adjoint sources in adjoint modelling)
    allocate_array_GPU(rtf_uz, nrec, nt);
    allocate_array_GPU(rtf_ux, nrec, nt);
    ...
    if (accu||grad){
        ...
        if (accu){
            ...
            allocate_array_GPU(accu_vz, snap_nt, snap_nz, snap_nx);
            allocate_array_GPU(accu_vx, snap_nt, snap_nz, snap_nx);
            allocate_array_GPU(accu_szz, snap_nt, snap_nz, snap_nx);
            allocate_array_GPU(accu_szx, snap_nt, snap_nz, snap_nx);
            allocate_array_GPU(accu_sxx, snap_nt, snap_nz, snap_nx);
        }
        if (grad){
            // Allocating medium gradient
            allocate_array_GPU(grad_lam, nz, nx);
            allocate_array_GPU(grad_mu, nz, nx);
            allocate_array_GPU(grad_rho, nz, nx);

            // Allocating variables for old material storages
            allocate_array_GPU(lam_copy, nz, nx);
            allocate_array_GPU(mu_copy, nz, nx);
            allocate_array_GPU(rho_copy, nz, nx);

            // Allocate Energy for reverse modelling
            allocate_array_GPU(We_adj, nz, nx); // used later as temporary gradient

            // Allocating medium gradient for each shot
            allocate_array_GPU(grad_lam_shot, snap_nz, snap_nx);
            allocate_array_GPU(grad_mu_shot, snap_nz, snap_nx);
            allocate_array_GPU(grad_rho_shot, snap_nz, snap_nx); 
